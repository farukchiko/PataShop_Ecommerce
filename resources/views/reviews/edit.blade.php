                ...
                <div class="bg-amber-50 px-8 py-6 border-b border-amber-100">
                    <div class="flex items-center gap-6">
                        <div class="w-24 h-24 rounded-2xl overflow-hidden bg-white border border-amber-200 shadow-sm flex-shrink-0">
                            @php $imgSrc = is_array($review->product->image) ? ($review->product->image[0] ?? 'default.jpg') : $review->product->image; @endphp
                            <img src="{{ asset('images/'.$imgSrc) }}" alt="{{ $review->product->name }}" class="w-full h-full object-cover">
                        </div>
                        <div>
                            <h3 class="text-xl font-bold text-amber-900 mb-1">{{ $review->product->name }}</h3>
                            <p class="text-sm font-semibold text-amber-700 mb-2">Order #{{ substr($review->order->id, 0, 8) }}</p>
                ...
                <div class="p-8">
                    <form action="{{ route('reviews.update', $review->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="flex items-center gap-2" x-data="{ rating: {{ old('rating', $review->rating) }}, hoverRating: 0 }">
                        ...
                        <div>
                            ...
                            <textarea id="comment" name="comment" rows="4" 
                                      ...
                                      placeholder="Ceritakan pengalaman Anda menggunakan produk ini..." required>{{ old('comment', $review->comment) }}</textarea>
                        ...
                        <div class="pt-6 border-t border-amber-100 flex items-center justify-between">
                            <p class="text-xs text-amber-600/70 max-w-sm">Perhatian: Jika Anda mengunggah media baru, semua foto/video ulasan sebelumnya akan dihapus.</p>
                            <div class="flex items-center gap-4">
                                <a href="{{ route('storefront.show', $review->product_id) }}" class="font-bold text-amber-700 hover:text-amber-900 transition-colors">Batal</a>
