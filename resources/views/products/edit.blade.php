...
@section('content')
    <div class="mb-8 flex items-center justify-between">
        <div>
            <h2 class="text-4xl font-black text-amber-900 tracking-tight">Edit Product</h2>
            <p class="text-amber-700/80 mt-2 font-medium">Update details and media for {{ $product->name }}.</p>
        </div>
    ...
    <form action="{{ route('products.update', $product->id) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 xl:grid-cols-3 gap-8" x-data="productForm()">
        @csrf
        @method('PUT')
            ...
            <div class="bg-white p-8 rounded-3xl shadow-sm border border-amber-100 relative overflow-hidden">
                ...
                <div class="space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $product->name) }}" class="w-full border-amber-200 focus:border-amber-500 focus:ring-amber-500 rounded-xl shadow-sm text-amber-900 p-3" required placeholder="e.g. Premium Chocolate Cake">
                    </div>

                    <div>
                        <label class="block text-sm font-bold text-amber-900 mb-2">Description <span class="text-red-500">*</span></label>
                        <textarea name="description" rows="6" class="w-full border-amber-200 focus:border-amber-500 focus:ring-amber-500 rounded-xl shadow-sm text-amber-900 p-3" required placeholder="Detailed description of the product...">{{ old('description', $product->description) }}</textarea>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white p-8 rounded-3xl shadow-sm border border-amber-100">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <div class="relative">
                            <input type="number" name="stock" value="{{ old('stock', $product->stock) }}" min="1" class="w-full border-amber-200 focus:border-amber-500 focus:ring-amber-500 rounded-xl shadow-sm text-amber-900 p-3 font-bold text-lg" required placeholder="50">
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="space-y-8 xl:sticky xl:top-8 h-fit">
            ...
            <div class="bg-amber-50 rounded-2xl p-5 mb-6 border border-amber-100">
                <p class="text-sm text-amber-800 font-medium">
                    Upload up to 5 <strong class="text-red-600">new</strong> images to completely replace the current ones. <strong>Leave blank</strong> if you want to keep the current images.
                </p>
            </div>
            ...
            <div class="w-full border-2 border-dashed border-amber-300 rounded-2xl p-8 text-center hover:bg-amber-50 transition-colors cursor-pointer group mb-6" @click="$refs.fileInput.click()">
                <div class="text-amber-500 group-hover:text-amber-700 transition-colors transform group-hover:scale-105 duration-200">
                    ...
                    <p class="font-black text-lg mb-1">Click to replace photos</p>
            ...
            <div x-show="files.length > 0" class="mb-6">
                <div class="flex items-center justify-between mb-3">
                    <h4 class="text-sm font-bold text-amber-900" x-text="`New Photos (${files.length}/5)`"></h4>
                </div>
                    ...
                    <template x-for="(file, index) in files" :key="index">
                        <div class="relative aspect-square rounded-xl overflow-hidden border-2 border-amber-200 shadow-sm group">
                            ...
                            <template x-if="index === 0">
                                <span class="absolute top-2 left-2 bg-amber-600 text-white text-xs font-black px-2.5 py-1 rounded shadow-md z-10">New Cover</span>
                            </template>
            ...
            <!-- Current Images -->
            <div x-show="files.length === 0" class="pt-2 border-t border-amber-100">
                <h4 class="text-sm font-bold text-amber-900 mb-4">Current Photos</h4>
                <div class="grid grid-cols-2 gap-4 opacity-80">
                    @if(is_array($product->image))
                        @foreach($product->image as $index => $img)
                            <div class="relative aspect-square rounded-xl overflow-hidden border-2 border-amber-200 shadow-sm">
                                <img src="{{ asset('images/'.$img) }}" class="w-full h-full object-cover">
                                @if($index === 0)
                                    <span class="absolute top-2 left-2 bg-amber-600/80 text-white text-xs font-black px-2.5 py-1 rounded shadow z-10">Cover</span>
                                @endif
                            </div>
                        @endforeach
                    ...
                    @else
                        <div class="col-span-2 text-sm text-gray-500 italic">No images currently set.</div>
                    @endif
                </div>
                </div>
            </div>
            ...
            <div class="bg-white p-8 rounded-3xl shadow-sm border border-amber-100">
                ...
                <p class="text-sm text-amber-700/80 font-medium mb-6">
                    Save your modifications and update the product in your storefront.
                </p>
    ...
    <script>
        function productForm() {
            return {
                ...
                formattedPrice: '{{ old("price") ? number_format(old("price"), 0, "", ".") : number_format($product->price, 0, "", ".") }}',
                files: [],
